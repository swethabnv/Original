...
<%@page import="com.wsnweb.form.FarmerGroupFarmerForm"%>
...
<% 
	...
	FarmerGroupFarmerForm farmerGroupfarmerForm = (FarmerGroupFarmerForm) request.getAttribute("FarmerGroupFarmerForm");
	...
	String farmerGroupName= "", farmerGroupType= "";
	String msg = "",cmd = "",idCard = "",fname="",lname="";
	double tmptarget = 0;
	long farmerGroupId =0;
	if(farmerGroupfarmerForm!=null){
		farmerGroupId = farmerGroupfarmerForm.getFarmerGroupId();
		farmerGroupName = farmerGroupfarmerForm.getFarmerGroupName();
		farmerGroupType = farmerGroupfarmerForm.getFarmerGroupType();
		tmptarget = farmerGroupfarmerForm.getTarget();
		msg = farmerGroupfarmerForm.getMsg()==null?"":farmerGroupfarmerForm.getMsg();
		cmd = farmerGroupfarmerForm.getCmd();
	}
%>
...
function loadData(){
	...
	document.farmerGroupFarmerForm.target.value= "<%=tmptarget%>";
    loadMenu("M10");
    showErrorMessage(); 
}
function searchFarmer(){
	    document.farmerGroupFarmerForm.cmd.value = "Search";
	...
	if(cmd == 'Delete'){
		...
		if (count > 0){
			if (confirm("ต้องการลบข้อมูลที่เลือกใช่หรือไม่ ?")) {
				document.farmerGroupFarmerForm.cmd.value="Delete";
				document.farmerGroupFarmerForm.action="FarmerGroupFarmer.do";
				document.forms["farmerGroupFarmerForm"].submit();
			}
		}else{
...
function showWindow()
  {	
     var new_window = window.open("<%=request.getContextPath()%>/FarmerGroupAdd.do?farmerGroupId=<%=farmerGroupId%>","popup", "toolbar=no, scrollbars=no, resizable=no, top=100, left=150, width=850, height=560");
	 new_window.focus();
...
<dcs:validateForm formName="farmerGroupFarmerForm" formAction="FarmerGroupFarmer.do" formBean="FarmerGroupFarmerForm">
		...
		<div class="navigator">
			<div class="inside">
	    	 	<ul>
					...
					<li><img src="<%=request.getContextPath()%>/images/arrow-navigator.png" width="4" height="8" /></li>
		           <li>การจัดการสมาชิกกลุ่มเกษตรกร</li>
		     	</ul>
		 	</div>
    	</div>	
    ...
    <div class="content">
    	<div class="inside">
        	...
        	<div class="content-search">
                ...
                <!-- <p>แก้ไขสมาชิกกลุ่มเกษตรกร</p>  -->
                 &nbsp;&nbsp;&nbsp;<span style="vertical-align: -7px;color: white;" ><font class="topic" >แก้ไขสมาชิกกลุ่มเกษตรกร</font></span> 
                </div>
	                <div class="line-input">
	                	<ul>
	                    	<li class="topic" style="width: 120px; font-weight: bold;" >ชื่อกลุ่มเกษตรกร : </li>
	                ...
	                <div class="line-input">
	                	<ul>
	                    	<li class="topic" style="width: 120px; font-weight: bold;" >สังกัดสหกรณ์ : </li>
	                    	<li class="topic" style="width: 300px; text-align: left;"><%="-".equals(farmerGroupType)?"ไม่สังกัด":farmerGroupType%></li>
	                    </ul>
	                </div>
			</div>
            ...
            <td><dcs:grid dataSource="<%= farmerGroupList %>" name="farmerGroupList" pageSize="<%=farmerGroupfarmerForm.getDisplayRow()%>" width="100%">
                 ...
                 <dcs:gridsorter/>                 
                 <dcs:checkboxcolumn name="delfarmerGroupName" dataField="idCard" headerText="" width="50"  HAlign="left" />
                 <dcs:textcolumn dataField="memberNo" headerText="เลขที่สมาชิก" width="100" sortable="true" HAlign="left" cssClass="tel-sr" />
                 ...
                 <dcs:textcolumn dataField="provinceName" headerText="จังหวัด" width="120" sortable="true" HAlign="left" cssClass="tel-sr" />
                 <dcs:textcolumn dataField="districtName" headerText="อำเภอ" width="120" sortable="true" HAlign="left" cssClass="tel-sr" />
                 <dcs:textcolumn dataField="subDistrictName" headerText="ตำบล" width="120" sortable="true" HAlign="left" cssClass="tel-sr" />
                 </dcs:grid>
            </td>
           	<% } %>
            </tr></table>
