...
<%@page import="com.wsnweb.form.FarmerGroupAddForm"%>
<head>
...
<% 
	...
	FarmerGroupAddForm farmerGroupfarmerForm = (FarmerGroupAddForm) request.getAttribute("FarmerGroupAddForm");
	...
	if(farmerGroupfarmerForm!=null){
		...
		tmptarget = farmerGroupfarmerForm.getTmptarget();
		msg = farmerGroupfarmerForm.getMsg()==null?"":farmerGroupfarmerForm.getMsg();
		cmd = farmerGroupfarmerForm.getCmd();
		idCard = farmerGroupfarmerForm.getIdCard();
		fname = farmerGroupfarmerForm.getFirstName();
		lname = farmerGroupfarmerForm.getLastName();
	}
%>
...
function loadData(){
	document.farmerGroupAddForm.farmerGroupId.value= "<%=farmerGroupId%>";
	document.farmerGroupAddForm.farmerGroupName.value= "<%=farmerGroupName%>";
    <% if(cmd !=null && "Search".equals(cmd)){ %>
         document.farmerGroupAddForm.idCard.value = "<%=idCard%>";
         document.farmerGroupAddForm.firstName.value = "<%=fname%>";
   		 document.farmerGroupAddForm.lastName.value = "<%=lname%>";
...
function searchFarmer(){
	    document.farmerGroupAddForm.cmd.value = "Search";
	    document.farmerGroupAddForm.action= "<%=request.getContextPath()%>/FarmerGroupAdd.do";
	    //document.forms["farmerGroupFarmerForm"].submit();
	   document.forms["farmerGroupAddForm"].submit();
}
	...
	if(cmd == 'Add'){
		...
		if (count > 0){
			if (confirm("ต้องการเพิ่มเกษตรกรที่เลือกเข้ากลุ่มใช่หรือไม่ ?")) {
				document.farmerGroupAddForm.cmd.value = "Add";
				document.farmerGroupAddForm.farmerGroupId.value = "<%=farmerGroupId%>";
				document.farmerGroupAddForm.action="<%=request.getContextPath()%>/FarmerGroupAdd.do";
				document.forms["farmerGroupAddForm"].submit();
			}
...
<dcs:validateForm formName="farmerGroupAddForm" formAction="FarmerGroupAdd.do" formBean="FarmerGroupAddForm">
...
<div class="main-inside" style="padding:15px;text-align: center;">
	<table id = "farmerTable" >
	    <tr>
	    	<td colspan="7" style="text-align: left;padding-left: 25px">
				<font style="font-weight:bold;">ชื่อกลุ่มเกษตรกร:</font> <%=farmerGroupName %>
			</td>
		</tr>
		<tr>
	...
	<div style="padding-left: 0px; margin-left:0px;">
            	...
            	<tr>
            	<%   if(farmerList!=null){ %>
            	<td><dcs:grid dataSource="<%= farmerList %>" name="farmerList" pageSize="<%=farmerGroupfarmerForm.getDisplayRow()%>" width="100%">
