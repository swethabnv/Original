...
function CalendarControl() {
	...
	var dayofweek = ['อา','จ','อ','พ','พฤ','ศ','ส'];
	var currentYear = now.getFullYear();  /// getFullYear() by Ngek for chrome 
	...
	function writeCalendar(tbDate) {
		...
		table += "<td align='left' class='month' width='30%'>"+months[selectedMonth]+"</td>";
		table += "<td align='right' class='month' width='30%'>"+(selectedYear+543)+"</td>";
	...
	function selectDate(sday) {
		txtbx.value=('0'+sday).slice(-2)+"/"+('0'+(selectedMonth+1)).slice(-2)+"/"+(selectedYear+543);
		if (txtbx.onchange!=null) {
			txtbx.onchange();
		}
	...
	function setDate(strDate) {
		if(strDate.length>0) {
			...
			selectedYear = Number(temp.substring(temp.indexOf("/")+1, temp.length)-543);
...
function showCalendar(txtbx) { 
    ...
    calendarControl.setDate(txtbx.value); // set textbox
    var popup = document.getElementById("CalendarPopup");
    popup.innerHTML=calendarControl.writeCalendar(txtbx);
	popup.style.top=y-tpos+"px";//+txtbx.offsetHeight+"px";  /// addd "px" by Ngek for chrome
	popup.style.left=x+txtbx.offsetWidth-175+"px"; /// addd "px" by Ngek for chrome
